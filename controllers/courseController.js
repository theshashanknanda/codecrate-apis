...
exports.deleteCourse = async (req, res, next) => {
  try {
    ...
    const deletedCartItems = await Cart.deleteMany({ courseId: courseId })
...
exports.addCourseToStudent = async (req, res, next) => {
  try {
    ...
    if (user.courses.includes(courseId)) {
      return res.status(400).json({
        success: false,
        message: 'Tutorial already enrolled',
      });
    }
...
exports.codeRunnerController = async (req, res) => {
  try {
     ...
     * 
     * // create unique directory for run
     * // const id = uuidv4()
     * // const tempDir = path.join('/tmp', id)
     * // fs.mkdirSync(tempDir)
     * 
     * // save user code in the unique directory as script.js
     ...
     * //   output = execSync(
     * //     `docker run --rm -v ${tempDir}:/usr/src/app -w /usr/src/app --network none --memory 256m --cpus 0.5 node:20-slim node script.js`,
